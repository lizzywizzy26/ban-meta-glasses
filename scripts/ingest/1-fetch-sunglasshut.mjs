...
//
// UNKNOWN going in (first-pass, generic extraction, same starting point
// David Clulow's script began with before real data showed its structure):
//   - Sunglass Hut is also owned by EssilorLuxottica (same group as
//     Ray-Ban and Vision Express), but that doesn't tell us anything about
//     which store-locator platform this particular site runs on.
//   - Whether any entry carries a Ray-Ban-Meta-specific signal at all, or
//     whether this is a generic "find any Sunglass Hut" locator the same
//     way Vision Express's first page turned out to be.
//   - The URL used here (store-locations, not store-locations/map) was
//     chosen because a map-based page is more likely to be JavaScript-
//     rendered with little in the initial HTML — if this comes back with
//     0 records, that's the likely reason, and the map URL would need
//     different handling (can't fix that blind, hence sending back the
//     raw HTML either way).
//
...
const SOURCE_URL = 'https://www.sunglasshut.com/uk/sunglasses/store-locations';
