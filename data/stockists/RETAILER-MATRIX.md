...
| **Vision Express Ireland** (`visionexpress.ie/opticians/ray-ban-meta`) | ✅ Confirmed (dedicated Ray-Ban Meta stockist page) | **Fetched and verified 16 Aug 2026: 6 real records**, all in Dublin (Blanchardstown, Henry Street, Tallaght, Liffey Valley, Clarehall, Balbriggan). Parser correctness confirmed against the real raw HTML: exactly 6 `<article class="store-tile">` elements exist in the page, matching 6 heading-links and 18 (6×3) address rows — nothing was missed by the extraction. **One caveat not yet resolved:** all 6 being in Dublin is a real, unexplained pattern — earlier research this campaign did found Vision Express Ireland also has branches in Cork, Galway, and Athlone, none of which appear here. Two explanations are both plausible and neither is confirmed: (a) this page is genuinely a curated "these specific stores stock Ray-Ban Meta" list (same directory-level judgment already applied to the UK Vision Express page — see decision above) and Ray-Ban Meta rollout in Ireland really is Dublin-only so far, or (b) the page silently geo-scopes to a default location with no signal of that in the static HTML this session could fetch (a live browser check would settle it, not attemptable from here). Recorded here rather than resolved — treat this batch as "confirmed Ray-Ban Meta stockists on this specific page," not as proof of Dublin-only availability nationally |
