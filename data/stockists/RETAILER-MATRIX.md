...
| **John Lewis** | ✅ Confirmed (29 models listed) | **Parser built and tested 15 Aug 2026** against a real captured response: 36 stores returned for one SKU, 21 with positive live stock (`verified_branch`), 15 "Not available" (`authorised_chain`, not dropped). Confirmed cookie-free via Incognito test — a plain public GET, no session needed | **Dynamic — solved.** `1-fetch-john-lewis.mjs` ready to run for real | High | Response is XML (not JSON as first assumed), parsed with regex (no new dependency). One real bug found and fixed during testing: shopping-centre branches (e.g. White City) have 4 address lines instead of 3, and a naive "3rd line = postcode" assumption silently grabbed "London" instead of the real postcode for those stores — fixed by taking the postcode from the *last* address line instead of a fixed position. Only queries one SKU so far (one colour/lens variant) — full coverage across John Lewis's ~29 Ray-Ban Meta variants needs more SKU IDs added to the script, not yet done |
